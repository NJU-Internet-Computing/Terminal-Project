...
var _sudokuComp = _interopRequireDefault(__webpack_require__(/*! ./sudokuComp/sudokuComp.js */ 13));var _mutations;function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}function _defineProperty(obj, key, value) {if (key in obj) {Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true });} else {obj[key] = value;}return obj;}
...
var store = new _vuex.default.Store({
  ...
  modules: {
    sudokuComp: _sudokuComp.default },

  mutations: mutations,
  actions: actions });

var getters = { //实时监听state值的变化(最新状态)
  getMode: function getMode(state) {//承载变化的mode的值
    return state.mode;
  },
  getdifficulty: function getdifficulty(state) {//承载变化的难度的值
    return state.difficulty;
  } };

var mutations = (_mutations = {
  chooseMode: function chooseMode(state) {//自定义改变state初始值的方法，这里面的参数除了state之外还可以再传额外的参数(变量或对象);
    state.mode = "PUZZLE";
  },
  chooseMiddle: function chooseMiddle(state) {//选择中等难度
    state.difficulty = 1;
  } }, _defineProperty(_mutations, "chooseMiddle", function chooseMiddle(
state) {//选择中等难度
  state.difficulty = 2;
}), _defineProperty(_mutations, "chooseHard", function chooseHard(
state) {//选择困难难度
  state.difficulty = 3;
}), _mutations);

var actions = {
  changeMode: function changeMode(context) {//自定义触发mutations里函数的方法，context与store 实例具有相同方法和属性
    context.commit('chooseMode');
  },
  changeMiddle: function changeMiddle(context) {//同上注释
    context.commit('chooseMiddle');
  },
  changeHard: function changeHard(context) {//同上注释
